...
makeChaosGame=function(type="points",...){
	return(component(type=type,...)+action.data(fun="runChaosGame",...))
}

#' @rdname ChaosGame
#' @export
runChaosGame=function(inputx=0,inputy=0,x=NULL,y=NULL,attractor=list(x=c(-1,-1,1,1),y=c(-1,1,1,-1)),decisions=NULL,runTime=1000,sampleSize=NULL,reuseInput=FALSE,decisionMode=1,optionsSet=NULL){
	if(reuseInput|is.null(x)){x=inputx}
	if(reuseInput|is.null(y)){y=inputy}
	lenx=length(x)
	if(length(decisions)<runTime){
		if(is.null(optionsSet)){
		...
		if(decisionMode==3){
			for(i in seq(from=length(decisions)+1,to=runTime)){
				if(i==1&&length(decisions)==0){
					decisions[1]=sample(optionsSet[[sample(seq_along(optionsSet),size=1)]],size=1)
				}else{
					decisions[i]=sample(optionsSet[[decisions[i-1]]],size=1)
				}
			}
		}
	}
